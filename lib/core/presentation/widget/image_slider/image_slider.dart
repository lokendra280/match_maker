...
class assetslider extends StatefulWidget {
  //All the assets on this Carousel.
  final List assets;
  ...
  //How to show the assets in the box. Default is cover
  final BoxFit boxFit;

  //Enable/Disable radius Border for the assets. Default is false
  final bool borderRadius;

  //Border Radius of the assets. Default is [Radius.circular(8.0)]
  final Radius? radius;
  ...
  @override
  State createState() => assetsliderState();
}

class assetsliderState extends State<assetslider> {
    ...
    if (widget.autoplay) {
      Timer.periodic(widget.autoplayDuration, (_) {
        if (_controller.hasClients) {
          if (_controller.page?.round() == widget.assets.length - 1) {
  ...
  Widget build(BuildContext context) {
    final listassets = widget.assets.map<Widget>(
      (netImage) {
    ...
    return Stack(
      ...
      if(widget.counterWidget==null)   Positioned(
          ...
          child: Container(
            ...
            child: Text(
              '${_currentImageIndex+1}/${widget.assets.length}',
